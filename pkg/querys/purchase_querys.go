...
var (
	...
	CarrierExist      = "SELECT EXISTS(SELECT 1 FROM carriers WHERE id = ?);"
	OrderStatusExist  = "SELECT EXISTS(SELECT 1 FROM order_status WHERE id = ?);"
	Warehouse         = "SELECT EXISTS(SELECT 1 FROM warehouses WHERE id = ?);"
	OrderNumberExist  = "SELECT EXISTS(SELECT 1 FROM purchase_orders WHERE order_number like ?);"
	TrackingCodeExist = "SELECT EXISTS(SELECT 1 FROM purchase_orders WHERE tracking_code like ?);"
)
